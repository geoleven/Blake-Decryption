...
static HashReturn Update32(hashState * state, const BitSequence * data,
...
static HashReturn Update64(hashState * state, const BitSequence * data,
		DataLength databitlen) {
	...
	if (databitlen > 0) {
		memcpy((void *) (state->data64 + left), (void *) data,
				(databitlen >> 3) & 0x7F);
		state->datalen = (left << 3) + databitlen;
...
static HashReturn Final32(hashState * state, BitSequence * hashval) {
	...
	if (state->datalen % 8 == 0) {
		...
		if ((state->datalen > 440) && (state->datalen < 447)) {
			...
			state->datalen = (state->datalen
					& (DataLength) 0xfffffffffffffff8ULL) + 8;
		} else {
			if (state->datalen < 440) {
				/* enough space to fill the block */
				state->t32[0] -= 440 - state->datalen;
